/*
 * CRAYON TURBO ENGINE v5.7.13 (L2-Resident Cache + Clean OMP)
 * ==============================================================================
...
/* ══════════════════════════════════════════════════════════════
 *  Core Tokenize Loop  (v5.7.13 — L2-resident cache, no prefetch overhead)
 * ══════════════════════════════════════════════════════════════ */
    ...
    while (pos < len) {
        ...
        if (isw) {
            ...
            if (__builtin_expect(wl <= MAX_WORD_LEN, 1)) {
                ...
                /* Compute hash — one memcpy for the common ≤7 char case */
                uint64_t key;
...
slow_word:;
                ...
                /* Word > MAX_WORD_LEN: raw DAT walk, no caching */
                size_t wp = ws;
                while (wp < we) {
                    int32_t tid; int ml = dat_match(text, we, wp, &tid);
...
/* ══════════════════════════════════════════════════════════════
 *  Persistent Worker Thread  (v5.7.13 — replaces OMP fork/join)
 *  ──────────────────────────────────────────────────────────────
...
static PyObject *py_get_hardware_info(PyObject *self, PyObject *args) {
    ...
    char info[320];
    snprintf(info,sizeof(info),"%s [Turbo/v5.7.13/64K-SA+%s/%s 64Ksets×2ways intcache=%u]",
             brand,
#if HAVE_AVX2
             "AVX2",
#else
...
static struct PyModuleDef moddef = {
    ...
    "CRAYON Turbo v5.7.13: 64K-SA+AVX2+OMP+numpy", -1, methods
};
